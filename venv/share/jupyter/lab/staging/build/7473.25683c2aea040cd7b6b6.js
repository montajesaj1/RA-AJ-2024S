(self["webpackChunk_jupyterlab_application_top"] = self["webpackChunk_jupyterlab_application_top"] || []).push([[7473],{
...
var fast_json_stable_stringify = __webpack_require__(35035);
...
function pick(obj, props) {
    ...
    for (const prop of props) {
        if ((0,vega_util_module/* hasOwnProperty */.nr)(obj, prop)) {
            copy[prop] = obj[prop];
        }
    }
    return copy;
 ...
 */
function hash(a) {
    if ((0,vega_util_module/* isNumber */.hj)(a)) {
        return a;
    }
    const str = (0,vega_util_module/* isString */.HD)(a) ? a : fast_json_stable_stringify_default()(a);
...
function deepMerge_(dest, src) {
    for (const property of keys(src)) {
        (0,vega_util_module/* writeConfig */.iL)(dest, property, src[property], true);
    }
}
...
function prefixGenerator(a) {
    ...
    for (const x of a) {
        const splitField = (0,vega_util_module/* splitAccessPath */._k)(x);
...
function accessPathWithDatum(path, datum = 'datum') {
    const pieces = (0,vega_util_module/* splitAccessPath */._k)(path);
    const prefixes = [];
    for (let i = 1; i <= pieces.length; i++) {
        const prefix = `[${pieces.slice(0, i).map(vega_util_module/* stringValue */.m8).join('][')}]`;
...
function flatAccessWithDatum(path, datum = 'datum') {
    return `${datum}[${(0,vega_util_module/* stringValue */.m8)((0,vega_util_module/* splitAccessPath */._k)(path).join('.'))}]`;
}
...
function replacePathInField(path) {
    return `${(0,vega_util_module/* splitAccessPath */._k)(path).map(escapePathAccess).join('\\.')}`;
}
/**
...
function removePathFromField(path) {
    return `${(0,vega_util_module/* splitAccessPath */._k)(path).join('.')}`;
}
/**
...
function accessPathDepth(path) {
    ...
    return (0,vega_util_module/* splitAccessPath */._k)(path).length;
}
/**
...
function isNumeric(value) {
    if ((0,vega_util_module/* isNumber */.hj)(value)) {
        return true;
    }
...
function isAggregateOp(a) {
    return (0,vega_util_module/* isString */.HD)(a) && !!AGGREGATE_OP_INDEX[a];
}
...
function isCountingAggregateOp(aggregate) {
    return (0,vega_util_module/* isString */.HD)(aggregate) && COUNTING_OPS.has(aggregate);
}
function isMinMaxOp(aggregate) {
    return (0,vega_util_module/* isString */.HD)(aggregate) && contains(['min', 'max'], aggregate);
}
...
function binToString(bin) {
    if ((0,vega_util_module/* isBoolean */.jn)(bin)) {
...
function isBinParams(bin) {
    return (0,vega_util_module/* isObject */.Kn)(bin);
}
...
function isText(v) {
    return (0,vega_util_module/* isString */.HD)(v) || ((0,vega_util_module/* isArray */.kJ)(v) && (0,vega_util_module/* isString */.HD)(v[0]));
}
...
function isDataRefUnionedDomain(domain) {
    if (!(0,vega_util_module/* isArray */.kJ)(domain)) {
...
function signalOrValueRefWithCondition(val) {
    const condition = (0,vega_util_module/* isArray */.kJ)(val.condition)
...
function exprFromSignalRefOrValue(ref) {
    ...
    return (0,vega_util_module/* stringValue */.m8)(ref);
}
function exprFromValueRefOrSignalRef(ref) {
    if (isSignalRef(ref)) {
        return ref.signal;
    }
    return (0,vega_util_module/* stringValue */.m8)(ref.value);
}
function signalOrStringValue(v) {
    if (isSignalRef(v)) {
        return v.signal;
    }
    return v == null ? null : (0,vega_util_module/* stringValue */.m8)(v);
}
...
function getStyleConfig(p, styles, styleConfigIndex) {
    styles = (0,vega_util_module/* array */.IX)(styles);
    let value;
...
function sortParams(orderDef, fieldRefOption) {
    return (0,vega_util_module/* array */.IX)(orderDef).reduce((s, orderChannelDef) => {
        var _a;
...
function mergeTitle(title1, title2) {
    if (deepEqual(title1, title2) || !title2) {
        ...
        return [...(0,vega_util_module/* array */.IX)(title1), ...(0,vega_util_module/* array */.IX)(title2)].join(', ');
    }
}
 ...
 */
const main = (0,vega_util_module/* logger */.kg)(vega_util_module/* Warn */.uU);
let current = main;
/**
...
function isDateTime(o) {
    if (o && (0,vega_util_module/* isObject */.Kn)(o)) {
...
function normalizeQuarter(q) {
    ...
    if ((0,vega_util_module/* isNumber */.hj)(q)) {
        if (q > 4) {
...
function normalizeMonth(m) {
    ...
    if ((0,vega_util_module/* isNumber */.hj)(m)) {
...
function normalizeDay(d) {
    ...
    if ((0,vega_util_module/* isNumber */.hj)(d)) {
...
function dateTimeParts(d, normalize) {
    ...
    if (d.month !== undefined) {
        ...
        parts.push((0,vega_util_module/* isNumber */.hj)(quarter) ? quarter * 3 : `${quarter}*3`);
    }
    else {
    ...
    if (d.date !== undefined) {
        ...
        parts.push((0,vega_util_module/* isNumber */.hj)(day) ? day + 1 : `${day}+1`);
    }
    else {
...
function normalizeTimeUnit(timeUnit) {
    ...
    let params;
    if ((0,vega_util_module/* isString */.HD)(timeUnit)) {
        params = {
            unit: timeUnit
        };
    }
    else if ((0,vega_util_module/* isObject */.Kn)(timeUnit)) {
...
function isFieldRangePredicate(predicate) {
    if (predicate === null || predicate === void 0 ? void 0 : predicate.field) {
        if ((0,vega_util_module/* isArray */.kJ)(predicate.range) && predicate.range.length === 2) {
            return true;
        }
...
function isFieldOneOfPredicate(predicate) {
    return (!!(predicate === null || predicate === void 0 ? void 0 : predicate.field) && ((0,vega_util_module/* isArray */.kJ)(predicate.oneOf) || (0,vega_util_module/* isArray */.kJ)(predicate.in)) // backward compatibility
    );
}
...
var vega_module_js_ = __webpack_require__(56498);
...
function isExtendedScheme(scheme) {
    return !(0,vega_util_module/* isString */.HD)(scheme) && !!scheme['name'];
}
...
function fieldInvalidPredicate(field, invalid = true) {
    return fieldValidPredicate((0,vega_util_module/* isString */.HD)(field) ? field : vgField(field, { expr: 'datum' }), !invalid);
}
...
function midPoint({ channel, channelDef, channel2Def, markDef, config, scaleName, scale, stack, offset, defaultRef, bandPosition }) {
    ...
    if ((0,vega_util_module/* isFunction */.mf)(defaultRef)) {
...
function numberFormat({ type, specifiedFormat, config, normalizeStack }) {
    ...
    if ((0,vega_util_module/* isString */.HD)(specifiedFormat)) {
...
function binNumberFormatExpr(field, format, formatType, config) {
    ...
    return formatExpr(field, (_a = ((0,vega_util_module/* isString */.HD)(format) ? format : undefined)) !== null && _a !== void 0 ? _a : config.numberFormat);
}
...
function timeFormatExpression({ field, timeUnit, format, formatType, rawTimeFormat, isUTCScale }) {
    if (!timeUnit || format) {
        ...
        format = (0,vega_util_module/* isString */.HD)(format) ? format : rawTimeFormat; // only use provided timeFormat if there is no timeUnit.
...
function isSortArray(sort) {
    return sort && (0,vega_util_module/* isArray */.kJ)(sort);
}
...
function isRepeatRef(field) {
    return field && !(0,vega_util_module/* isString */.HD)(field) && 'repeat' in field;
}
...
function hasConditionalFieldDef(channelDef) {
    ...
    return !!condition && !(0,vega_util_module/* isArray */.kJ)(condition) && isFieldDef(condition);
}
function hasConditionalFieldOrDatumDef(channelDef) {
    const condition = channelDef === null || channelDef === void 0 ? void 0 : channelDef['condition'];
    return !!condition && !(0,vega_util_module/* isArray */.kJ)(condition) && isFieldOrDatumDef(condition);
}
function hasConditionalValueDef(channelDef) {
    const condition = channelDef === null || channelDef === void 0 ? void 0 : channelDef['condition'];
    return !!condition && ((0,vega_util_module/* isArray */.kJ)(condition) || isValueDef(condition));
}
...
function isNumericDataDef(cd) {
    return isDatumDef(cd) && (0,vega_util_module/* isNumber */.hj)(cd.datum);
}
...
function defaultType(fieldDef, channel) {
    ...
    if (isSortableFieldDef(fieldDef) && (0,vega_util_module/* isArray */.kJ)(fieldDef.sort)) {
        return 'ordinal';
    }
...
function initChannelDef(channelDef, channel, config, opt = {}) {
    if ((0,vega_util_module/* isString */.HD)(channelDef) || (0,vega_util_module/* isNumber */.hj)(channelDef) || (0,vega_util_module/* isBoolean */.jn)(channelDef)) {
        const primitiveType = (0,vega_util_module/* isString */.HD)(channelDef) ? 'string' : (0,vega_util_module/* isNumber */.hj)(channelDef) ? 'number' : 'boolean';
...
function initDatumDef(datumDef) {
    ...
    const { datum } = datumDef;
    type = (0,vega_util_module/* isNumber */.hj)(datum) ? 'quantitative' : (0,vega_util_module/* isString */.HD)(datum) ? 'nominal' : isDateTime(datum) ? 'temporal' : undefined;
...
function initFieldDef(fd, channel, { compositeMark = false } = {}) {
    ...
    if (isSortableFieldDef(fieldDef) && (0,vega_util_module/* isString */.HD)(fieldDef.sort)) {
        const { sort } = fieldDef;
...
function valueExpr(v, { timeUnit, type, wrapTime, undefinedIfExprNotRequired }) {
    ...
    else if ((0,vega_util_module/* isString */.HD)(v) || (0,vega_util_module/* isNumber */.hj)(v)) {
        if (isTime) {
            expr = `datetime(${stringify(v)})`;
            if (isLocalSingleTimeUnit(unit)) {
                // for single timeUnit, we will use dateTimeToExpr to convert number/string to match the timeUnit
                if (((0,vega_util_module/* isNumber */.hj)(v) && v < 10000) || ((0,vega_util_module/* isString */.HD)(v) && isNaN(Date.parse(v)))) {
...
function channelHasField(encoding, channel) {
    ...
    if (channelDef) {
        if ((0,vega_util_module/* isArray */.kJ)(channelDef)) {
...
function initEncoding(encoding, mark, filled, config) {
    ...
    for (let channel of UNIT_CHANNELS) {
        ...
        if (channel === DETAIL ||
            (channel === ORDER && !(0,vega_util_module/* isArray */.kJ)(channelDef) && !isValueDef(channelDef)) ||
            (channel === TOOLTIP && (0,vega_util_module/* isArray */.kJ)(channelDef))) {
            if (channelDef) {
                // Array of fieldDefs for detail channel (or production rule)
                normalizedEncoding[channel] = (0,vega_util_module/* array */.IX)(channelDef).reduce((defs, fieldDef) => {
                    if (!isFieldDef(fieldDef)) {
...
function fieldDefs(encoding) {
    ...
    for (const channel of keys(encoding)) {
        if (channelHasField(encoding, channel)) {
            const channelDef = encoding[channel];
            const channelDefArray = (0,vega_util_module/* array */.IX)(channelDef);
...
function forEach(mapping, f, thisArg) {
    ...
    for (const channel of keys(mapping)) {
        ...
        if ((0,vega_util_module/* isArray */.kJ)(el)) {
...
function reduce(mapping, f, init, thisArg) {
    ...
    return keys(mapping).reduce((r, channel) => {
        ...
        if ((0,vega_util_module/* isArray */.kJ)(map)) {
...
function pathGroupingFields(mark, encoding) {
    return keys(encoding).reduce((details, channel) => {
        switch (channel) {
            ...
            case KEY: {
                const channelDef = encoding[channel];
                if ((0,vega_util_module/* isArray */.kJ)(channelDef) || isFieldDef(channelDef)) {
                    for (const fieldDef of (0,vega_util_module/* array */.IX)(channelDef)) {
...
function filterTooltipWithAggregatedField(oldEncoding) {
    ...
    if ((0,vega_util_module/* isArray */.kJ)(tooltip)) {
    ...
    if ((0,vega_util_module/* isArray */.kJ)(customTooltipWithoutAggregatedField) && customTooltipWithoutAggregatedField.length === 1) {
...
function partLayerMixins(markDef, part, compositeMarkConfig, partBaseSpec) {
    ...
    if (markDef[part] || (markDef[part] === undefined && compositeMarkConfig[part])) {
        return [
            Object.assign(Object.assign({}, partBaseSpec), { mark: Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, compositeMarkConfig[part]), (clip ? { clip } : {})), (color ? { color } : {})), (opacity ? { opacity } : {})), (isMarkDef(partBaseSpec.mark) ? partBaseSpec.mark : { type: partBaseSpec.mark })), { style: `${mark}-${String(part)}` }), ((0,vega_util_module/* isBoolean */.jn)(markDef[part]) ? {} : markDef[part])) })
        ];
    }
    return [];
}
...
function getBoxPlotType(extent) {
    if ((0,vega_util_module/* isNumber */.hj)(extent)) {
        return 'tukey';
    }
...
function normalizeBoxPlot(spec, { config }) {
    ...
    const boxLayers = [
        ...
        ...makeBoxPlotMidTick({
            partName: 'median',
            mark: Object.assign(Object.assign(Object.assign({ type: 'tick', invalid }, ((0,vega_util_module/* isObject */.Kn)(config.boxplot.median) && config.boxplot.median.color ? { color: config.boxplot.median.color } : {})), (sizeValue ? { size: sizeValue } : {})), { orient: ticksOrient, aria: false }),
...
function isLegendStreamBinding(bind) {
    return isLegendBinding(bind) && (0,vega_util_module/* isObject */.Kn)(bind);
}
...
function isStep(size) {
    return (0,vega_util_module/* isObject */.Kn)(size) && size['step'] !== undefined;
}
    ...
    for (const prop of COMPOSITION_LAYOUT_PROPERTIES) {
        if (spec[prop] !== undefined) {
            if (prop === 'spacing') {
                ...
                layout[prop] = (0,vega_util_module/* isNumber */.hj)(spacing)
                    ? spacing
                    : {
...
function colorSignalConfig(color = {}) {
    return {
        signals: [
            {
                name: 'color',
                value: (0,vega_util_module/* isObject */.Kn)(color) ? Object.assign(Object.assign({}, DEFAULT_COLOR), color) : DEFAULT_COLOR
            }
        ],
...
function fontSizeSignalConfig(fontSize) {
    return {
        signals: [
            {
                name: 'fontSize',
                value: (0,vega_util_module/* isObject */.Kn)(fontSize) ? Object.assign(Object.assign({}, DEFAULT_FONT_SIZE), fontSize) : DEFAULT_FONT_SIZE
            }
        ],
        text: {
...
function initConfig(specifiedConfig = {}) {
    ...
    const mergedConfig = (0,vega_util_module/* mergeConfig */.fE)({}, duplicate(config_defaultConfig), font ? fontConfig(font) : {}, color ? colorSignalConfig(color) : {}, fontSize ? fontSizeSignalConfig(fontSize) : {}, restConfig || {});
...
function stripAndRedirectConfig(config) {
    ...
    if (config.mark) {
        ...
        if (config.mark.tooltip && (0,vega_util_module/* isObject */.Kn)(config.mark.tooltip)) {
    ...
    for (const prop in config) {
        if ((0,vega_util_module/* isObject */.Kn)(config[prop]) && isEmpty(config[prop])) {
            delete config[prop];
        }
    }
...
function isLayerRepeatSpec(spec) {
    return !(0,vega_util_module/* isArray */.kJ)(spec.repeat) && spec.repeat['layer'];
}
...
function stack(m, encoding) {
    ...
    const stackBy = NONPOSITION_CHANNELS.reduce((sc, channel) => {
        ...
        if (channel !== 'tooltip' && channelHasField(encoding, channel)) {
            const channelDef = encoding[channel];
            for (const cDef of (0,vega_util_module/* array */.IX)(channelDef)) {
    ...
    if (stackedFieldDef.stack !== undefined) {
        if ((0,vega_util_module/* isBoolean */.jn)(stackedFieldDef.stack)) {
...
function getPointOverlay(markDef, markConfig = {}, encoding) {
    if (markDef.point === 'transparent') {
        ...
        return (0,vega_util_module/* isObject */.Kn)(markDef.point) ? markDef.point : {};
    }
        ...
        if (markConfig.point || encoding.shape) {
            ...
            return (0,vega_util_module/* isObject */.Kn)(markConfig.point) ? markConfig.point : {};
        }
...
function replaceRepeaterInMapping(mapping, repeater) {
    ...
    for (const channel in mapping) {
        if ((0,vega_util_module/* hasOwnProperty */.nr)(mapping, channel)) {
...
class RuleForRangedLineNormalizer {
    ...
    run(spec, params, normalize) {
        ...
        return normalize(Object.assign(Object.assign({}, spec), { mark: (0,vega_util_module/* isObject */.Kn)(mark) ? Object.assign(Object.assign({}, mark), { type: 'rule' }) : 'rule' }), params);
    }
}
...
class CoreNormalizer extends SpecMapper {
    ...
    mapNonLayerRepeat(spec, params) {
        ...
        if (!(0,vega_util_module/* isArray */.kJ)(repeat) && spec.columns) {
        ...
        const concat = [];
        const { repeater = {}, repeaterPrefix = '' } = params;
        const row = (!(0,vega_util_module/* isArray */.kJ)(repeat) && repeat.row) || [repeater ? repeater.row : null];
        const column = (!(0,vega_util_module/* isArray */.kJ)(repeat) && repeat.column) || [repeater ? repeater.column : null];
        const repeatValues = ((0,vega_util_module/* isArray */.kJ)(repeat) && repeat) || [repeater ? repeater.repeat : null];
        // cross product
            ...
            for (const rowValue of row) {
                for (const columnValue of column) {
                    ...
                    const childName = (childSpec.name || '') +
                        repeaterPrefix +
                        'child__' +
                        ((0,vega_util_module/* isArray */.kJ)(repeat)
                            ? `${varName(repeatValue)}`
        ...
        const columns = (0,vega_util_module/* isArray */.kJ)(repeat) ? spec.columns : repeat.column ? repeat.column.length : 1;
    ...
    if (parentEncoding) {
        ...
        for (const channel of channels) {
            ...
            if (isFieldOrDatumDef(channelDef)) {
                ...
                (0,vega_util_module/* isArray */.kJ)(parentChannelDef)) {
...
function _normalizeAutoSize(autosize) {
    return (0,vega_util_module/* isString */.HD)(autosize) ? { type: autosize } : autosize !== null && autosize !== void 0 ? autosize : {};
}
/**
...
var vega_event_selector_module = __webpack_require__(83082);
...
function assembleInit(init, isExpr = true, wrap = vega_util_module/* identity */.yR) {
    if ((0,vega_util_module/* isArray */.kJ)(init)) {
            ...
            on: [
                {
                    ...
                    update: `modify(${(0,vega_util_module/* stringValue */.m8)(selCmpt.name + STORE)}, ${modifyExpr})`
                }
            ]
        });
    }
...
function assembleFacetSignals(model, signals) {
    if (model.component.selection && keys(model.component.selection).length) {
        const name = (0,vega_util_module/* stringValue */.m8)(model.getName('cell'));
        signals.unshift({
            name: 'facet',
            value: {},
            on: [
                {
                    events: (0,vega_event_selector_module/* parseSelector */.r)('mousemove', 'scope'),
...
function assembleTopLevelSignals(model, signals) {
    ...
    for (const selCmpt of vals((_a = model.component.selection) !== null && _a !== void 0 ? _a : {})) {
        const name = selCmpt.name;
        const store = (0,vega_util_module/* stringValue */.m8)(name + STORE);
        ...
        if (hasSg.length === 0) {
            ...
            signals.push({
                name: selCmpt.name,
                update: `${VL_SELECTION_RESOLVE}(${store}, ${(0,vega_util_module/* stringValue */.m8)(resolve)}${isPoint}`
            });
        }
        hasSelections = true;
        for (const c of selectionCompilers) {
...
function assembleSelectionScaleDomain(model, extent, scaleCmpt, domain) {
    ...
    return {
        signal: hasContinuousDomain(scaleCmpt.get('type')) && (0,vega_util_module/* isArray */.kJ)(domain) && domain[0] > domain[1]
...
const project = {
    ...
    parse: (model, selCmpt, selDef) => {
        ...
        const init = selDef.value !== undefined
            ? (0,vega_util_module/* array */.IX)(selDef.value)
            : null;
        // If no explicit projection (either fields or encodings) is specified, set some defaults.
        // If an initial value is set, try to infer projections.
        let { fields, encodings } = ((0,vega_util_module/* isObject */.Kn)(selDef.select) ? selDef.select : {});
        if (!fields && !encodings && init) {
            for (const initVal of init) {
                ...
                if (!(0,vega_util_module/* isObject */.Kn)(initVal)) {
                    continue;
                }
        ...
        if (init) {
            selCmpt.init = init.map((v) => {
                ...
                return proj.items.map(p => ((0,vega_util_module/* isObject */.Kn)(v) ? (v[p.channel] !== undefined ? v[p.channel] : v[p.field]) : v));
            });
        }
    ...
    topLevelSignals: (model, selCmpt, signals) => {
        ...
        if (update.indexOf(VL_SELECTION_RESOLVE) >= 0) {
            namedSg.update = `{${bound
                .map(proj => `${(0,vega_util_module/* stringValue */.m8)(replacePathInField(proj.field))}: ${proj.signals.data}`)
                .join(', ')}}`;
        }
        else {
            for (const proj of bound) {
                const mapping = `${(0,vega_util_module/* stringValue */.m8)(replacePathInField(proj.field))}: ${proj.signals.data}`;
...
function scales_domain(model, channel) {
    const scale = (0,vega_util_module/* stringValue */.m8)(model.scaleName(channel));
    return `domain(${scale})`;
}
...
const interval = {
    ...
    signals: (model, selCmpt, signals) => {
        ...
        if (selCmpt.translate && !hasScales) {
            const filterExpr = `!event.item || event.item.mark.name !== ${(0,vega_util_module/* stringValue */.m8)(name + BRUSH)}`;
            events(selCmpt, (on, evt) => {
                var _a;
                var _b;
                const filters = (0,vega_util_module/* array */.IX)(((_a = (_b = evt.between[0]).filter) !== null && _a !== void 0 ? _a : (_b.filter = [])));
        ...
        selCmpt.project.items.forEach((proj, i) => {
            ...
            const scaleName = (0,vega_util_module/* stringValue */.m8)(model.scaleName(channel));
            const scaleType = model.getScaleComponent(channel).get('type');
    ...
    marks: (model, selCmpt, marks) => {
        ...
        const store = `data(${(0,vega_util_module/* stringValue */.m8)(selCmpt.name + STORE)})`;
...
const point = {
    ...
    signals: (model, selCmpt, signals) => {
        ...
        if (selCmpt.project.hasSelectionId) {
            update += `${SELECTION_ID}: ${datum}[${(0,vega_util_module/* stringValue */.m8)(SELECTION_ID)}]`;
        }
        else {
            const values = project.items
                ...
                return (fieldDef === null || fieldDef === void 0 ? void 0 : fieldDef.bin)
                    ? `[${datum}[${(0,vega_util_module/* stringValue */.m8)(model.vgField(p.channel, {}))}], ` +
                        `${datum}[${(0,vega_util_module/* stringValue */.m8)(model.vgField(p.channel, { binSuffix: 'end' }))}]]`
                    : `${datum}[${(0,vega_util_module/* stringValue */.m8)(p.field)}]`;
            })
                .join(', ');
...
function wrapCondition(model, channelDef, vgChannel, refFn) {
    ...
    if (condition) {
        const conditions = (0,vega_util_module/* array */.IX)(condition);
...
function tooltip(model, opt = {}) {
    ...
    if ((0,vega_util_module/* isArray */.kJ)(channelDef)) {
        ...
        return wrapCondition(model, channelDef, 'tooltip', cDef => {
            ...
            if ((0,vega_util_module/* isString */.HD)(markTooltip)) {
                return { value: markTooltip };
            }
            else if ((0,vega_util_module/* isObject */.Kn)(markTooltip)) {
...
function tooltipData(encoding, stack, config, { reactiveGeom } = {}) {
    ...
    function add(fDef, channel) {
        ...
        const key = (0,vega_util_module/* array */.IX)(title).join(', ');
        let value;
        if (isXorY(channel)) {
            const channel2 = channel === 'x' ? 'x2' : 'y2';
...
function defaultSizeRef(sizeChannel, scaleName, scale, config, bandSize) {
    ...
    if (scale) {
        ...
        if (isVgRangeStep(scaleRange) && (0,vega_util_module/* isNumber */.hj)(scaleRange.step)) {
...
function positionAndSize(fieldDef, channel, model) {
    ...
    if (vgSizeChannel) {
        ...
        return {
            ...
            [vgChannel2]: (0,vega_util_module/* isArray */.kJ)(posRef)
        ...
        if (test) {
            return {
                [channel]: [
                    ...
                    { test, value: null },
                    ...(0,vega_util_module/* array */.IX)(valueRef)
                ]
            };
        }
    }
                        ...
                        ? [
                            {
                                ...
                                update: `datum && item().mark.marktype !== 'group' ? ${datum}[${(0,vega_util_module/* stringValue */.m8)(p.field)}] : null`
                            }
                        ]
...
const clear = {
    ...
    parse: (model, selCmpt) => {
        if (selCmpt.clear) {
            selCmpt.clear = (0,vega_util_module/* isString */.HD)(selCmpt.clear) ? (0,vega_event_selector_module/* parseSelector */.r)(selCmpt.clear, 'view') : selCmpt.clear;
        }
    },
    topLevelSignals: (model, selCmpt, signals) => {
...
const legendBindings = {
    ...
    parse: (model, selCmpt, selDef) => {
        ...
        selDef_.select = (0,vega_util_module/* isString */.HD)(selDef_.select)
        ...
        if ((0,vega_module_js_.isObject)(selDef.select) && (selDef.select.on || selDef.select.clear)) {
            ...
            for (const evt of selCmpt.events) {
                evt.filter = (0,vega_util_module/* array */.IX)((_a = evt.filter) !== null && _a !== void 0 ? _a : []);
        ...
        const stream = (0,vega_util_module/* isString */.HD)(evt) ? (0,vega_event_selector_module/* parseSelector */.r)(evt, 'view') : (0,vega_util_module/* array */.IX)(evt);
...
const translate = {
    ...
    signals: (model, selCmpt, signals) => {
        ...
        const { x, y } = selCmpt.project.hasChannel;
        let events = (0,vega_event_selector_module/* parseSelector */.r)(selCmpt.translate, 'scope');
        if (!hasScales) {
...
const zoom = {
    ...
    signals: (model, selCmpt, signals) => {
        ...
        const { x, y } = selCmpt.project.hasChannel;
        const sx = (0,vega_util_module/* stringValue */.m8)(model.scaleName(X));
        const sy = (0,vega_util_module/* stringValue */.m8)(model.scaleName(Y));
        let events = (0,vega_event_selector_module/* parseSelector */.r)(selCmpt.zoom, 'scope');
        if (!hasScales) {
...
function unitName(model, { escape } = { escape: true }) {
    let name = escape ? (0,vega_util_module/* stringValue */.m8)(model.name) : model.name;
    const facetModel = getFacetModel(model);
    if (facetModel) {
        const { facet } = facetModel;
        for (const channel of FACET_CHANNELS) {
            if (facet[channel]) {
                name += ` + '__facet_${channel}_' + (facet[${(0,vega_util_module/* stringValue */.m8)(facetModel.vgField(channel))}])`;
            }
        }
    }
    return name;
...
//# sourceMappingURL=index.js.map
// EXTERNAL MODULE: ./node_modules/vega-expression/build/vega-expression.module.js
var vega_expression_module = __webpack_require__(25693);
...
function getDependentFields(expression) {
    const ast = (0,vega_expression_module/* parseExpression */.BJ)(expression);
...
function parseUnitSelection(model, selDefs) {
    ...
    for (const def of selDefs) {
        ...
        const type = (0,vega_util_module/* isString */.HD)(selDef) ? selDef : selDef.type;
        const defaults = (0,vega_util_module/* isObject */.Kn)(selDef) ? duplicate(selDef) : { type };
        ...
        const selCmpt = (selCmpts[name] = Object.assign(Object.assign({}, defaults), { name,
            type, init: def.value, bind: def.bind, events: (0,vega_util_module/* isString */.HD)(defaults.on) ? (0,vega_event_selector_module/* parseSelector */.r)(defaults.on, 'scope') : (0,vega_util_module/* array */.IX)(duplicate(defaults.on)) }));
        for (const c of selectionCompilers) {
...
function parseSelectionPredicate(model, pred, dfnode, datum = 'datum') {
    const name = (0,vega_util_module/* isString */.HD)(pred) ? pred : pred.param;
    const vname = varName(name);
    const store = (0,vega_util_module/* stringValue */.m8)(vname + STORE);
    let selCmpt;
    try {
    ...
    const resolve = selCmpt.resolve === 'global' ? ')' : `, ${(0,vega_util_module/* stringValue */.m8)(selCmpt.resolve)})`;
...
function parseSelectionExtent(model, name, extent) {
    ...
    if (!encoding && !field) {
        ...
        if (selCmpt.project.items.length > 1) {
            warn('A "field" or "encoding" must be specified when using a selection as a scale domain. ' +
                `Using "field": ${(0,vega_util_module/* stringValue */.m8)(field)}.`);
        }
    }
        ...
        if (!encodings.length || encodings.length > 1) {
            ...
            warn((!encodings.length ? 'No ' : 'Multiple ') +
                `matching ${(0,vega_util_module/* stringValue */.m8)(encoding)} encoding found for selection ${(0,vega_util_module/* stringValue */.m8)(extent.param)}. ` +
                `Using "field": ${(0,vega_util_module/* stringValue */.m8)(field)}.`);
        }
        else {
    ...
    return `${selCmpt.name}[${(0,vega_util_module/* stringValue */.m8)(replacePathInField(field))}]`;
}
...
function expression(model, filterOp, node) {
    return logicalExpr(filterOp, (predicate) => {
        if ((0,vega_util_module/* isString */.HD)(predicate)) {
            return predicate;
        }
...
function assembleTitle(title, config) {
    ...
    if ((0,vega_util_module/* isArray */.kJ)(title) && !isText(title)) {
...
function getAxisConfigStyle(axisConfigTypes, config) {
    ...
    for (const configType of axisConfigTypes) {
        ...
        if (style) {
            style = (0,vega_util_module/* array */.IX)(style);
...
function defaultLabelOverlap(type, scaleType, hasTimeUnit, sort) {
    ...
    if ((hasTimeUnit && !(0,vega_util_module/* isObject */.Kn)(sort)) || (type !== 'nominal' && type !== 'ordinal')) {
...
function values(axis, fieldOrDatumDef) {
    ...
    if ((0,vega_util_module/* isArray */.kJ)(vals)) {
...
function getSort(facetFieldDef, channel) {
    ...
    else if ((0,vega_util_module/* isArray */.kJ)(sort)) {
        return {
...
function symbols(symbolsSpec, { fieldOrDatumDef, model, channel, legendCmpt, legendType }) {
    ...
    if (out.fill) {
        ...
        if (channel === 'fill' || (filled && channel === COLOR)) {
            ...
            else if ((0,vega_util_module/* isArray */.kJ)(out.fill)) {
    ...
    if (out.stroke) {
        if (channel === 'stroke' || (!filled && channel === COLOR)) {
            ...
            else if ((0,vega_util_module/* isArray */.kJ)(out.stroke)) {
...
function getConditionValue(channelDef, reducer) {
    if (hasConditionalValueDef(channelDef)) {
        return (0,vega_util_module/* array */.IX)(channelDef.condition).reduce(reducer, channelDef.value);
    }
    else if (isValueDef(channelDef)) {
...
function selectedCondition(model, legendCmpt, fieldDef) {
    ...
    if (!(selections === null || selections === void 0 ? void 0 : selections.length))
        return undefined;
    const field = (0,vega_util_module/* stringValue */.m8)(fieldDef.field);
    return selections
        .map(name => {
        const store = (0,vega_util_module/* stringValue */.m8)(varName(name) + STORE);
...
function mergeIfNoConflict(first, second) {
    const allPropertiesShared = every(PROJECTION_PROPERTIES, prop => {
        ...
        if (!(0,vega_util_module/* hasOwnProperty */.nr)(first.explicit, prop) && !(0,vega_util_module/* hasOwnProperty */.nr)(second.explicit, prop)) {
            return true;
        }
        // both have property and an equal value for property
        if ((0,vega_util_module/* hasOwnProperty */.nr)(first.explicit, prop) &&
            (0,vega_util_module/* hasOwnProperty */.nr)(second.explicit, prop) &&
...
function createBinComponent(t, bin, model) {
    ...
    let span;
    if (isBinTransform(t)) {
        as = (0,vega_util_module/* isString */.HD)(t.as) ? [t.as, `${t.as}_end`] : [t.as[0], t.as[1]];
    }
    else {
        ...
        for (const channel of FACET_CHANNELS) {
            ...
            if (fieldDef) {
                ...
                this[channel] = Object.assign({ name: model.getName(`${channel}_domain`), fields: [vgField(fieldDef), ...(isBinning(bin) ? [vgField(fieldDef, { binSuffix: 'end' })] : [])] }, (isSortField(sort)
                    ? { sortField: sort }
                    : (0,vega_util_module/* isArray */.kJ)(sort)
    ...
    forEachLeaf(transform.filter, filter => {
        ...
        if (isFieldPredicate(filter)) {
            ...
            if (val) {
                ...
                else if ((0,vega_util_module/* isNumber */.hj)(val)) {
                    implicit[filter.field] = 'number';
                }
                else if ((0,vega_util_module/* isString */.HD)(val)) {
...
function isValidAsArray(as) {
    return (0,vega_util_module/* isArray */.kJ)(as) && as.every(s => (0,vega_util_module/* isString */.HD)(s)) && as.length > 1;
}
class StackNode extends DataFlowNode {
    ...
    static makeFromTransform(parent, stackTransform) {
        ...
        else if ((0,vega_util_module/* isString */.HD)(as)) {
    ...
    static makeFromEncoding(parent, model) {
        ...
        let sort;
        if ((0,vega_util_module/* isArray */.kJ)(orderDef) || isFieldDef(orderDef)) {
            ...
            return makeImplicit([
                {
                    ...
                    sort: sort === true || !(0,vega_util_module/* isObject */.Kn)(sort)
                        ? {
                            field: model.vgField(channel, {}),
...
function canUseUnaggregatedDomain(fieldDef, scaleType) {
    ...
    if ((0,vega_util_module/* isString */.HD)(aggregate) && !SHARED_DOMAIN_OPS.has(aggregate)) {
        return {
            valid: false,
...
function mergeDomains(domains) {
    ...
    if (uniqueDomains.length === 0) {
        ...
        if (isDataRefDomain(domain) && sorts.length > 0) {
            ...
            if (sorts.length > 1) {
                ...
                if ((0,vega_util_module/* isObject */.Kn)(sort) && 'field' in sort) {
    ...
    const unionDomainSorts = unique(sorts.map(s => {
        if (isBoolean(s) || !('op' in s) || ((0,vega_util_module/* isString */.HD)(s.op) && s.op in MULTIDOMAIN_SORT_OP_INDEX)) {
            return s;
        }
...
function getFieldFromDomain(domain) {
    if (isDataRefDomain(domain) && (0,vega_util_module/* isString */.HD)(domain.field)) {
        return domain.field;
    }
    else if (isDataRefUnionedDomain(domain)) {
        let field;
        for (const nonUnionDomain of domain.fields) {
            if (isDataRefDomain(nonUnionDomain) && (0,vega_util_module/* isString */.HD)(nonUnionDomain.field)) {
                if (!field) {
        ...
        return (0,vega_util_module/* isString */.HD)(field) ? field : undefined;
    }
    return undefined;
}
...
function assembleScaleRange(scaleRange, scaleName, channel, model) {
    ...
    else if ((0,vega_util_module/* isObject */.Kn)(scaleRange) && isDataRefDomain(scaleRange)) {
...
class ScaleComponent extends Split {
    ...
    domainDefinitelyIncludesZero() {
        ...
        return some(this.get('domains'), d => (0,vega_util_module/* isArray */.kJ)(d) && d.length === 2 && d[0] <= 0 && d[1] >= 0);
    }
}
//# sourceMappingURL=component.js.map
...
function getBinStepSignal(model, channel) {
    ...
    if (fieldDef === null || fieldDef === void 0 ? void 0 : fieldDef.bin) {
        ...
        if ((0,vega_util_module/* isObject */.Kn)(bin) && bin.binned && bin.step !== undefined) {
...
function parseRangeForChannel(channel, model) {
            ...
            if (!supportedByScaleType) {
                ...
                switch (property) {
                    case 'range': {
                        ...
                        if ((0,vega_util_module/* isArray */.kJ)(range)) {
                            if (isXorY(channel)) {
                        ...
                        else if ((0,vega_util_module/* isObject */.Kn)(range)) {
                            return makeExplicit({
                                data: model.requestDataName(DataSourceType.Main),
                                field: range.field,
    ...
    if ((rangeMin !== undefined || rangeMax !== undefined) &&
        ...
        (0,vega_util_module/* isArray */.kJ)(d) &&
        d.length === 2) {
...
function defaultContinuousToDiscreteCount(scaleType, config, domain, channel) {
    switch (scaleType) {
        ...
        case 'threshold':
            if (domain !== undefined && (0,vega_util_module/* isArray */.kJ)(domain)) {
...
function sizeRangeMax(mark, size, model, config) {
    ...
    switch (mark) {
        ...
        case 'tick': {
            ...
            if ((0,vega_util_module/* isNumber */.hj)(min)) {
                return min - 1;
            }
            else {
        ...
        case 'circle': {
            ...
            if ((0,vega_util_module/* isNumber */.hj)(pointStep)) {
...
function nice(scaleType, channel, specifiedDomain, domainMin, domainMax, fieldOrDatumDef) {
    ...
    if (((_a = getFieldDef(fieldOrDatumDef)) === null || _a === void 0 ? void 0 : _a.bin) ||
        (0,vega_util_module/* isArray */.kJ)(specifiedDomain) ||
        domainMax != null ||
        domainMin != null ||
...
function zero(channel, fieldDef, specifiedDomain, markDef, scaleType, scaleConfig, hasSecondaryRangeChannel) {
    ...
    if (hasCustomDomain) {
        if (hasContinuousDomain(scaleType)) {
            if ((0,vega_util_module/* isArray */.kJ)(specifiedDomain)) {
...
class GeoJSONNode extends DataFlowNode {
    ...
    dependentFields() {
        var _a;
        const fields = ((_a = this.fields) !== null && _a !== void 0 ? _a : []).filter(vega_util_module/* isString */.HD);
...
class GeoPointNode extends DataFlowNode {
    ...
    dependentFields() {
        return new Set(this.fields.filter(vega_util_module/* isString */.HD));
    }
    producedFields() {
...
class LookupNode extends DataFlowNode {
    ...
    producedFields() {
        return new Set(this.transform.as ? (0,vega_util_module/* array */.IX)(this.transform.as) : this.transform.from.fields);
    }
    hash() {
    ...
    assemble() {
        ...
        if (this.transform.from.fields) {
            ...
            foreign = Object.assign({ values: this.transform.from.fields }, (this.transform.as ? { as: (0,vega_util_module/* array */.IX)(this.transform.as) } : {}));
        }
        else {
            // lookup full record and nest it
            let asName = this.transform.as;
            if (!(0,vega_util_module/* isString */.HD)(asName)) {
...
function parseFacetHeader(model, channel) {
    ...
    if (model.channelHasField(channel)) {
        ...
        if (child.component.layoutHeaders[channel].title) {
            ...
            title = (0,vega_util_module/* isArray */.kJ)(title) ? title.join(', ') : title;
...
class FacetModel extends ModelWithField {
    ...
    facetSortFields(channel) {
        ...
        if (fieldDef) {
            ...
            else if ((0,vega_util_module/* isArray */.kJ)(fieldDef.sort)) {
    ...
    facetSortOrder(channel) {
        ...
        if (fieldDef) {
            const { sort } = fieldDef;
            const order = (isSortField(sort) ? sort.order : !(0,vega_util_module/* isArray */.kJ)(sort) && sort) || 'ascending';
            return [order];
        }
        return [];
    }
...
function defaultSize(model) {
    ...
    if (markPropOrConfig !== undefined) {
        ...
        if (scaleRange && isVgRangeStep(scaleRange) && (0,vega_util_module/* isNumber */.hj)(scaleRange.step)) {
...
function mark_getSort(model) {
    ...
    const order = encoding.order;
    if ((!(0,vega_util_module/* isArray */.kJ)(order) && isValueDef(order) && isNullOrFalse(order.value)) ||
        (!order && isNullOrFalse(getMarkPropOrConfig('order', markDef, config)))) {
        return undefined;
    }
    else if (((0,vega_util_module/* isArray */.kJ)(order) || isFieldDef(order)) && !stack) {
        ...
        if (isFieldDef(dimensionChannelDef)) {
            ...
            if ((0,vega_util_module/* isArray */.kJ)(s)) {
                return {
...
class LayerModel extends Model {
    ...
    assembleGroupStyle() {
        ...
        for (const child of this.children) {
            for (const style of (0,vega_util_module/* array */.IX)(child.assembleGroupStyle())) {
...
function compile(inputSpec, opt = {}) {
    ...
    try {
        ...
        const config = initConfig((0,vega_util_module/* mergeConfig */.fE)(opt.config, inputSpec.config));
...
function getTopLevelProperties(inputSpec, autosize, config, model) {
    ...
    else if ((0,vega_util_module/* isString */.HD)(autosize)) {
...
//# sourceMappingURL=7473.25683c2aea040cd7b6b6.js.map?v=25683c2aea040cd7b6b6