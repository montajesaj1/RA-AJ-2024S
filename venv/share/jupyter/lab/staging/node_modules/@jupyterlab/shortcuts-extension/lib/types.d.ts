...
export declare namespace CommandIDs {
    const editBinding = "shortcuts:edit-keybinding";
    const addBinding = "shortcuts:add-keybinding";
    const deleteBinding = "shortcuts:delete-keybinding";
    const toggleSelectors = "shortcuts:toggle-selectors";
    const resetAll = "shortcuts:reset-all";
}
/**
 ...
 */
export declare namespace IShortcutUI {
    /**
     * The column IDs which are also used for sorting; should not be translated.
     */
    export type ColumnId = 'label' | 'selector' | 'category' | 'source' | 'command';
     ...
     */
    export type ActionRequest = IChangeKebindingRequest | IAddKeybindingRequest | IToggleSelectorsRequest | IResetAllRequest;
    /**
     * A bundle of actions and objects passed down from the extension entry point.
     */
    export interface IExternalBundle {
        translator: ITranslator;
        getSettings: () => Promise<ISettingRegistry.ISettings<IShortcutsSettingsLayout>>;
 ...
 */
export interface IShortcutRegistry extends ReadonlyMap<string, IShortcutTarget> {
    /**
